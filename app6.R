...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Edit Table", value = "edit_tab", fluidRow(
      column(
        6, 
        h5("Instruction: Double-click a cell to edit. Fill all fields to add a new row."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$finish, {
    ...
    # Remove any empty rows from the table
    final_data <- rv$data[apply(rv$data, 1, function(row) any(trimws(row) != "")), ]
